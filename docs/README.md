...
${{\displaystyle{\mathcal{N}}(\mu ,\Sigma )}}$ ${{\displaystyle{\mathcal{N}}(\mu ,\Sigma )}}$, the distribution of some neural network features of the images generated by the GAN and ${{\displaystyle{\mathcal{N}}(\mu_{w},\Sigma _{w})}}$${{\displaystyle{\mathcal{N}}(\mu_{w},\Sigma_{w})}}$ the distribution of the same neural network features from the real images used to train the GAN.

Mathematically this can be written as ${\displaystyle{\mathcal{FID}} =||\mu -\mu_{w}||_{2}^{2}+}$ 
${\displaystyle{\mathrm{tr}}(\Sigma + \Sigma_{w} - 2(\Sigma^{\frac{1}{2}} \Sigma_{w} \Sigma^{\frac{1}{2}})^{\frac{1}{2}})}$
